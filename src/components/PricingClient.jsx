...
export default function PricingClient() {
  ...
  const plans = [
    { name: "Monthly Plan", price: "₹7,999 / month + GST", desc: [
    ...
    { name: "Quarterly Plan", price: "₹22,999 / 3 months + GST", desc: [
    ...
    { name: "Half-Yearly Plan", price: "₹44,999 / 6 months + GST", desc: [
    ...
    { name: "Yearly Plan", price: "₹89,999 / 12 months + GST", desc: [
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(180deg, #000000 0%, #9a753e 80%, #111111 100%)" }}
    >
      <Header />
          ...
          {plans.map((p) => (
            <button
              ...
              className={`px-6 py-2 rounded-md text-white font-medium transition-all duration-300 ${
                selectedPlan === p.name ? "bg-[#9a753e]" : "bg-[#1f1f1f] hover:bg-[#9a753e]/70"
              }`}
            >
              {p.name}
            </button>
        ...
        <div className="flex justify-center items-start gap-10">
          {plans.map((plan) => (
            <motion.div
              ...
              animate={{ opacity: selectedPlan === plan.name ? 1 : 0.8, scale: selectedPlan === plan.name ? 1.02 : 1 }}
              whileHover={{ scale: 1.02, boxShadow: "0px 0px 30px rgba(0,0,0,0.3)" }}
